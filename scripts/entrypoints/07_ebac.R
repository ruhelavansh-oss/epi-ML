...
self_path <- if (length(file_arg) > 0) sub('^--file=', '', file_arg[[1]]) else file.path('scripts', 'entrypoints', '07_ebac.R')
...
setwd(project_root)
source(file.path(project_root, 'surveillance/ebac/07_ebac.R'), local = FALSE, chdir = FALSE)
