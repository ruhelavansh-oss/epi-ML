...
# Expects encrypted tarball at data/private/encrypted/data_bundle.tar.gz.enc
# Writes decrypted data into data/private/runtime/ (ignored by git).

PROJECT_ROOT="${PROJECT_ROOT:-$(pwd)}"
ENC_FILE="${PROJECT_ROOT}/data/private/encrypted/data_bundle.tar.gz.enc"
RUNTIME_DIR="${PROJECT_ROOT}/data/private/runtime"
KEY="${SECRET_KEY:-}"

if [[ -z "${KEY}" ]]; then
  echo "SECRET_KEY is not set." >&2
  exit 1
fi
if [[ ! -f "${ENC_FILE}" ]]; then
...
mkdir -p "${RUNTIME_DIR}"
TMP_TAR="$(mktemp /tmp/data_bundle.XXXXXX.tar.gz)"
