...
# test_vars <- list(
#   wtpumf = pumf$wtpumf,
#   alc03  = pumf$alc03[!is.na(pumf$alc03) & pumf$alc03 <= 7],
#   alc06  = pumf$alc06[!is.na(pumf$alc06) & pumf$alc06 <= 30]
# )
...
# qqnorm(sample(pumf$wtpumf, 5000), main = "QQ: wtpumf vs Normal")
# qqline(sample(pumf$wtpumf, 5000), col = "red")

# alc03_filtered <- pumf$alc03[!is.na(pumf$alc03) & pumf$alc03 <= 7]
...
# alc06_filtered <- pumf$alc06[!is.na(pumf$alc06) & pumf$alc06 <= 30]
...
# qqnorm(log(sample(pumf$wtpumf, 5000)), main = "QQ: log(wtpumf) vs Normal")
# qqline(log(sample(pumf$wtpumf, 5000)), col = "red")
...
# - CPADS frequency codes (e.g., alc03/alc06) are ordinal category codes, not continuous.
# =============================================================================
#!/usr/bin/env Rscript
# =============================================================================
# 04_distributions.R — Phase 4: Distribution Fitting & Testing
...
test_vars <- list(
  wtpumf = pumf$wtpumf,
  alc03 = pumf$alc03[!is.na(pumf$alc03) & pumf$alc03 >= 1 & pumf$alc03 <= 7],
  alc06 = pumf$alc06[!is.na(pumf$alc06) & pumf$alc06 >= 1 & pumf$alc06 <= 6]
)
...
wt_vec <- pumf$wtpumf[!is.na(pumf$wtpumf)]
if (length(wt_vec) >= 10) {
  wt_samp <- sample(wt_vec, min(5000, length(wt_vec)))
  qqnorm(wt_samp, main = "QQ: wtpumf vs Normal")
  qqline(wt_samp, col = "red")
} else {
  plot.new()
  title("QQ: wtpumf (insufficient data)")
}

# QQ: alc03 code vs Normal (ordinal code)
alc03_filtered <- pumf$alc03[
  !is.na(pumf$alc03) & pumf$alc03 >= 1 & pumf$alc03 <= 7
]
...
alc06_filtered <- pumf$alc06[
  !is.na(pumf$alc06) & pumf$alc06 >= 1 & pumf$alc06 <= 6
]
if (length(alc06_filtered) >= 10) {
...
wt_pos <- pumf$wtpumf[!is.na(pumf$wtpumf) & pumf$wtpumf > 0]
if (length(wt_pos) >= 10) {
  wt_pos_samp <- sample(wt_pos, min(5000, length(wt_pos)))
  qqnorm(log(wt_pos_samp), main = "QQ: log(wtpumf) vs Normal")
  qqline(log(wt_pos_samp), col = "red")
} else {
  plot.new()
  title("QQ: log(wtpumf) (insufficient data)")
}
...
for (bv in binary_vars) {
  n_nm <- sum(!is.na(pumf[[bv]]))

  est <- svymean(as.formula(paste0("~", bv)), pumf_svy, na.rm = TRUE)
  ci <- confint(est)
...
# # --- Variables to test (treat as numeric diagnostics only) ---
# # wtpumf: positive continuous weights
...
# # QQ: weights vs Normal
# wt_samp <- sample(pumf$wtpumf[!is.na(pumf$wtpumf)], min(5000, sum(!is.na(pumf$wtpumf))))
# qqnorm(wt_samp, main = "QQ: wtpumf vs Normal")
# qqline(wt_samp, col = "red")

# # QQ: alc03 code vs Normal (ordinal code)
# alc03_filtered <- pumf$alc03[!is.na(pumf$alc03) & pumf$alc03 >= 1 & pumf$alc03 <= 7]
# alc03_samp <- sample(alc03_filtered, min(5000, length(alc03_filtered)))
# qqnorm(alc03_samp, main = "QQ: alc03 (ordinal code) vs Normal")
# qqline(alc03_samp, col = "red")

# # QQ: alc06 code vs Normal (ordinal code)
# alc06_filtered <- pumf$alc06[!is.na(pumf$alc06) & pumf$alc06 >= 1 & pumf$alc06 <= 6]
# if (length(alc06_filtered) > 10) {
#   alc06_samp <- sample(alc06_filtered, min(5000, length(alc06_filtered)))
#   qqnorm(alc06_samp, main = "QQ: alc06 (ordinal code) vs Normal")
#   qqline(alc06_samp, col = "red")
