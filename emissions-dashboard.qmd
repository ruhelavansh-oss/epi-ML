...
This page summarizes tracked cloud compute emissions for epi-ML runs and modules.
...
build_recommendations_from_cloud <- function(modules_df, runs_df) {
  ...
  out <- rbind(
    ...
    data.frame(
      ...
      rationale = sprintf("This module contributes %.1f%% of tracked cloud-run emissions.", top_module$emissions_share_pct[[1]]),
      stringsAsFactors = FALSE
    )
  )
...
if (has_data) {
  ...
  events <- events[events$run_source == "cloud", , drop = FALSE]
  runs <- runs[runs$run_source == "cloud", , drop = FALSE]
  ...
  if (nrow(events) > 0) {
    ...
    recommendations <- build_recommendations_from_cloud(modules, runs)
  }
...
if (!has_data) {
  cat(
    "No cloud emissions data is available yet.\n\n",
    "Run the GitHub Actions deploy workflow with emissions tracking, then refresh this page.",
    sep = ""
  )
} else {
